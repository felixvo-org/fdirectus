...
import { AssetsService, ItemsService } from '../../src/services';
...
describe('Integration Tests', () => {
	...
	describe('getAsset', () => {
		const directusFile = {
			storage: 'local',
			filename_disk: 'video.mp4',
			type: 'video/mp4',
		};

		describe('Range: 0-99 (max length: 1000)', async () => {
			const assetsService = new AssetsService({
				schema: { collections: {}, relations: [] },
			});

			tracker.on.select('directus_files').responseOnce(directusFile);

			await assetsService.getAsset('6107c897-9182-40f7-b22e-4f044d1258d2', {}, { start: 0, end: 99 });
		});
	});
